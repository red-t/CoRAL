...
class Interval:
    ...
    def encompasses(self, y: Interval) -> bool:
        """Check if an interval is fully contained within this one."""
        return self.chr == y.chr and self.start <= y.start and self.end >= y.end
...
@dataclass
class CNInterval(Interval):
    cn: float


@dataclass
class CycleInterval(Interval):
    strand: Strand
    cn: float
...
class CycleSolution:
    ...
    def num_walks(self) -> int:
        return len(self.walks.cycles) + len(self.walks.paths)


class InitialSolution(NamedTuple):
    """Container for storing initial solution state (generated by a heuristic
    method or incomplete solver run), to be passed to another model."""
...
class Solver(str, enum.Enum):
    """Enum for specifying the solver to pass to the MIQCP model used for
    cycle decomposition."""
...
class SingleArmInfo:
    ...
    interval: Interval
    size: int
    segs: list[CNInterval] = field(default_factory=list)
...
class ReconstructedCycle:
    cycle_id: int
    is_cyclic: bool
    segments: list[CycleInterval] = field(default_factory=list)

    def __str__(self) -> str:
        tag = "Cyclic" if self.is_cyclic else "Path"
        return f"{tag} {self.cycle_id}: {self.segments}"

    @property
    def total_cn_weighted_length(self) -> float:
        return sum(seg.cn * len(seg) for seg in self.segments)
